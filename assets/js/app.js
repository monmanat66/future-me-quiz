...
(function () {
  ...
  function resize() {
    ...
    makeStars(); // สร้างดาวใหม่เมื่อขนาดจอเปลี่ยน
  }
  ...
  function makeStars() {
    ...
    let N = Math.round(area / 9000);            // ความหนาแน่นดาวตามขนาดจอ
    N = Math.max(80, Math.min(N, 220));         // จำกัดช่วงเพื่อความเนียน
    stars = Array.from({ length: N }, () => ({
      x: Math.random() * c.width,
      y: Math.random() * c.height,
      r: Math.random() * 1.1 + 0.5,             // รัศมีดวงดาว
      s: Math.random() * (0.003 - 0.0008) + 0.0008, // ความถี่กระพริบ "ช้าลง"
      p: Math.random() * Math.PI * 2            // phase สุ่มให้ไม่พร้อมกัน
    }));
  }

  // ----- ดาวตกช้า ๆ -----
  const meteors = [];
  let nextSpawn = 0; // เวลา spawn ครั้งถัดไป (ms จาก performance.now())

  function spawnMeteor(now) {
    // สุ่มให้ดาวตกเข้ามาจาก "ขอบบน" หรือ "ขอบซ้าย"
    ...
    if (fromTop) {
      ...
      y = -margin;
      angle = Math.random() * 0.35 + 0.6; // มุมลงขวา
    } else {
      x = -margin;
      y = Math.random() * c.height * 0.5;
      angle = Math.random() * 0.25 + 0.75; // มุมลงขวา
    }

    // ความเร็วช้า ๆ (px ต่อ ms) + ยาวของหาง + อายุ
    const speed = (Math.random() * 0.20 + 0.10) * dpr; // 0.10–0.30 px/ms
    const vx = Math.cos(angle) * speed;
    const vy = Math.sin(angle) * speed;
    const len = (Math.random() * 140 + 120) * dpr;      // ความยาวหาง
    const ttl = Math.random() * 2000 + 2200;            // อายุ 2.2–4.2 วิ
    const width = (Math.random() * 1.5 + 0.8) * dpr;    // ความหนาเส้น

    meteors.push({ x, y, vx, vy, len, ttl, born: now, w: width });
  }

  // ----- เริ่มต้น -----
  resize(); // set canvas + makeStars()
  ...
  (function loop(now) {
    ...
    // วาดดาวพื้นหลัง (กระพริบช้า)
    for (const s of stars) {
      const alpha = 0.5 + Math.sin(now * s.s + s.p) * 0.35; // ค่อย ๆ สว่าง-มืด
    ...
    // สุ่มเกิดดาวตก (ทุก 2–6 วินาที, สูงสุดพร้อมกัน 2 ดวง)
    if (now > nextSpawn && meteors.length < 2) {
      spawnMeteor(now);
      nextSpawn = now + (Math.random() * 4000 + 2000); // 2–6s
    }

    // อัปเดต + วาดดาวตก
    ctx.save();
    ctx.globalCompositeOperation = 'lighter'; // ให้หางดูเรืองแสง
    for (let i = meteors.length - 1; i >= 0; i--) {
      ...
      const over =
        life > m.ttl ||
        m.x > c.width + 100 * dpr ||
        m.y > c.height + 100 * dpr;

      if (over) { meteors.splice(i, 1); continue; }

      // คำนวณจุดปลายหาง (ทิศย้อนจากความเร็ว)
      const nx = -m.vx, ny = -m.vy;
      const trail = m.len * (1 - life / m.ttl * 0.6); // หางสั้นลงเล็กน้อยตามเวลา
...
if (startBtnEl) {
  startBtnEl.addEventListener('click', () => { show('quiz'); initWizard(); });
}
...
const questions = [
  ...
  {
    type: 'abcd', title: 'งานวันนี้ให้คุณรู้สึกอย่างไร?',
    A: 'สนุกและมีสีสัน', B: 'ได้ความรู้/แรงบันดาลใจ', C: 'รู้สึกอบอุ่น/เจอเพื่อนใหม่', D: 'ตื่นเต้นและอยากมาร่วมอีก'
  },
  {
    type: 'abcd', title: 'ส่วนที่คุณชอบที่สุดในงานคือ?',
    A: 'Workshop/กิจกรรม', B: 'บรรยากาศ/การตกแต่ง', C: 'ร้านค้า/บูธ', D: 'การดูดวง/กิจกรรมสายมู'
  },
  {
    type: 'abcd', title: 'คุณอยากให้ปรับปรุงอะไรในครั้งต่อไป?',
    A: 'พื้นที่/การจัดโซน', B: 'เวลากิจกรรม', C: 'การประชาสัมพันธ์', D: 'อยากให้มีร้าน/กิจกรรมมากขึ้น'
  },
];

const resultPack = {
  A: {
    title: 'PLANNER ACHIEVER', key: 'ความพยายาม · ความมั่นคง · ความสำเร็จ',
  ...
  B: {
    title: 'DREAMER CREATOR', key: 'ความคิดสร้างสรรค์ · จินตนาการ · แรงบันดาลใจ',
  ...
  C: {
    title: 'CONNECTOR NURTURER', key: 'ความสัมพันธ์ · การให้ · การสนับสนุน',
  ...
  D: {
    title: 'EXPLORER LEARNER', key: 'การผจญภัย · การเรียนรู้ · ความกล้า',
...
function render() {
  ...
  if (inputField) {
    inputField.value = '';
    inputField.oninput = null; // ป้องกันซ้ำ
  }
...
function select(val) {
  const q = questions[idx];
  if (q.type === 'radio') { // เก็บค่าข้อความจริง
...
async function saveToFirestoreMinimal(pk, top, score) {
  try {
    ...
    const docData = {
      ...
      // meta
      result: { title: pk.title, key: pk.key, topLetter: top, score },
...
function computeResult() {
  ...
  const rk = document.getElementById('resultKeywords');
  ...
  if (rk) rk.textContent = pk.key;
  if (rm) rm.textContent = pk.msg[Math.min(imgIndex, pk.msg.length - 1)];
  show('result');

  // บันทึก Firestore (Q1,2,3,11,12,13)
  saveToFirestoreMinimal(pk, top, score)
